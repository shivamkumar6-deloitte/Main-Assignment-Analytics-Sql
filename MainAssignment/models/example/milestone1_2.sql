...
select na.code ,mu.name, max(na.nav) as maximumNav,min(na.nav) as minimumNav  from "ANALYTICS"."MAINASSIGNEMT"."navhistory" na, "ANALYTICS"."MAINASSIGNEMT"."mutualfund" mu where na.code = mu.code group by  na.code, mu.name